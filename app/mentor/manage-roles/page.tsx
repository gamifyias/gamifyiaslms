...
type Role = "student" | "mentor" | "admin" | "null"
...
export default function MentorRolePage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data } = await supabase
        ...
        .neq("role", "admin") // mentor never touches admins
        .order("created_at")
  ...
  const changeRole = async (user: Profile, newRole: Role) => {
    // 🚫 mentor cannot assign admin
    if (newRole === "admin") return

    setWorkingId(user.id)

    // 1️⃣ Update ENUM role
    ...
    // ⚠️ null = no access → no extra table
    ...
    toast({
      title: "Role Updated",
      description: `User role set to ${ROLE_LABEL[newRole]}`,
    })
            ...
            <Card key={user.id}>
              ...
              <CardContent className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  {(["student", "mentor", "null"] as Role[]).map(
                    (role) => (
                      <Button
                        key={role}
                        size="sm"
